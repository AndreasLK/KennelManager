...
class ScaleListener(
    private val zoomableContent: View, // Now the inner FrameLayout (zoomable_content)
    ...
    override fun onScale(detector: ScaleGestureDetector): Boolean {
        ...
        val newScaleFactor = (scaleHolder.scaleFactor * adjustedScaleFactor).coerceIn(0.3f, 2.0f)
        scaleHolder.scaleFactor = newScaleFactor

        zoomableContent.apply {
            // Pivot relative to the zoomableContent
            pivotX = detector.focusX - this.x
            pivotY = detector.focusY - this.y
